...
export const getUserProfile = async (req, res) => {
    try {
        //req.user comes directly from the auth middleware 
        const user = await User.findById(req.user._id);

        if(user) {
            console.log(`Profile accessed for the User ID: ${user._id}, Email: ${user.email}`);
            res.status(200).json({
                _id: user._id,
                name: user.name,
                email: user.email,
            ...
            console.warn(`Profile access failed: User not found for ID ${req.user._id}`);
